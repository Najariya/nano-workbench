...
| IMP-028 | In progress | P2 | Meetings | Improve meeting notes capture. | Build `6.8.14` adds meeting-note start/stop, first-use speech disclosure, local transcript capture, AI notes with action items, Memos saving, transcript copy, Markdown export, helper regression coverage, and Doctor voice-readiness checks; manual Chrome mic QA remains. |
...
| IMP-030 | In progress | P1 | Screenshots | Add quick screenshot capture. | Build `6.8.14` keeps screenshot capture as a dedicated composer camera button next to voice input, opens visible/full-page choices, previews/saves locally, and has static wiring coverage in `tests/screenshot-composer-button.test.js`; manual Chrome button QA remains. |
| IMP-031 | In progress | P1 | Voice | Add Momo-style voice capture refinements. | Build `6.8.14` adds quick voice memos with cleanup/action items, saved local transcripts, copy/export actions, speech-recognition disclosure, helper regression coverage, and Doctor/health voice-readiness checks; manual mic QA remains. |
| IMP-032 | In progress | P1 | Screenshot OCR | Improve screenshot OCR quality. | Build `6.8.14` reads tall screenshots in OCR slices, crops empty margins, upscales text-focused slices within a pixel budget, adds stronger contrast prep, extends `tests/screenshot-long-page.test.js`, and reports screenshot/OCR readiness in Doctor; OCR quality still requires manual verification on real captured pages. |
...
| IMP-032 | In progress | Improve screenshot OCR quality. | Captured screenshots are useful only if text remains legible enough for OCR and summary. | Tall screenshots are sliced before OCR instead of downscaled as one long image, OCR inputs crop empty margins and normalize contrast, automated slice/scale coverage exists, and Doctor reports screenshot capture/image-OCR readiness; manual OCR quality QA remains for real pages. |
